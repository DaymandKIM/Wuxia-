"""옛 시트 스트립의 살색·옷색을 새 시트 톤에 맞춘다 (v2.76.4, 사용자: "권기 중 피부색이 오묘하게 다른 게 있더라").
...
**옷색**(v2.76.6 "걷고 뛰는 거 봐봐"): 옛 시트는 허리띠·바지가 파랑 (80,92,140), 새 시트는 회청 (88,80,112). 파란 옷 픽셀
(b>r+20·b>g+10, 단 기운 이펙트는 밝고 g가 높아 b>150·g>110 제외)을 같은 방식으로 CLOTH_TARGET에 맞춘다.
사용: python skinmatch.py [키...]  (기본: run qipunch qipunchb kickside2 kickround2 kickhigh2 dashfly dashland)
...
OLD = ['run', 'qipunch', 'qipunchb', 'kickside2', 'kickround2', 'kickhigh2', 'dashfly', 'dashland']
def skin_mask(F):
    a = F[..., 3] > 0; r, g, b = F[..., 0], F[..., 1], F[..., 2]
    return a & (r > 140) & ((r - g) > 40) & ((g - b) > 15) & (b < 170)
TARGET = np.array([203., 138., 125.])     # 새 시트(주먹4·발차기3) 얼굴 밝은 30% 평균 — 불그레한 황갈
CLOTH_TARGET = np.array([88., 80., 112.]) # 새 시트 허리띠·바지 밝은 색 (회청)
def cloth_mask(F):
    a = F[..., 3] > 0; r, g, b = F[..., 0], F[..., 1], F[..., 2]
    return a & (b > r + 20) & (b > g + 10) & (b <= 150) & (g <= 110)
def shift(A, m, target, label):
    ...
    return f'{label} {src.round(0)}→{target} {int(m.sum())}px'
if __name__ == '__main__':
    keys = sys.argv[1:] or OLD
    target = TARGET
    print('기준 살색(새 시트 밝은 살색)', target)
    for k in keys:
        p = f'assets/{k}.png'; A = np.array(Image.open(p).convert('RGBA')).astype(int); notes = []
        m = skin_mask(A)
        if m.any(): notes.append(shift(A, m, target, '살색'))
        c = cloth_mask(A)
        if c.any(): notes.append(shift(A, c, CLOTH_TARGET, '옷'))
