...
setTimeout(()=>{
  ...
  ok(w.codexCount()>0 && Object.keys(S.inv).length>0,'도감 '+w.codexCount()+'종 · 주머니에 쌓였다');
  // 2) 합성 수동
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      const S3=w3.eval('S');
      ok(S3.equip.weapon.k==='gauntlet' && S3.equip.weapon.g===4 && S3.itemLv.gauntlet[4]===7 && S3.equip.armor===null && S3.itemLv.ring[3]===9,'옛 저장: 등급 잘림, 자리 강화 7·장비 lv 9는 아이템 레벨로 이월, 없는 종류 버림');
      ok(S3.inv.gauntlet[0]===2 && (S3.codex.gauntlet&16) && !S3.inv.sword && !S3.codex.fan,'v2.70.1 이월: 옛 검·도·철선이 권갑으로 합쳐진다(주머니·도감)');
      // 7) 패널
      ...
      ok(d3.querySelectorAll('.eqcard').length===30,'무기 탭 카드 6종×5등급');
