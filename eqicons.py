...
시트: 1줄 6칸(권갑·검·도·창·봉·부채 순), 순마젠타 배경, 칸 사이 얇은 어두운 세로선. 칸은 균등 6분할(선은 배경 판정으로 걷힘).
...
def main(grade, path):
    ...
    for i, k in enumerate(KINDS):
        ...
        lab, n = ndi.label(cell); sizes = ndi.sum(np.ones(lab.shape), lab, range(1, n + 1))
        keep = [j + 1 for j, s in enumerate(sizes) if s >= 150]
