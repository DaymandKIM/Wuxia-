...
if n:
    ...
    big = (y1-y0) > H*0.09 or (x1-x0) > W*0.16
    if big:
        # 안개·수풀처럼 큰 밝은 덩어리는 ✦ 가 아니다 — 건드리면 그림에 구멍이 난다
        print('✦ 아님(너무 큼 %dx%d) — 그대로 둔다' % (x1-x0, y1-y0))
    elif kind == 'ink':
        a[y0:y1, x0:x1, :3] = 0; a[y0:y1, x0:x1, 3] = 255   # 먹은 검게
        print('✦ 지움 (%d,%d %dx%d)' % (x0, y0, x1-x0, y1-y0))
    else:
        # 테두리·일러스트는 **왼쪽으로 같은 크기 띠를 복사**해 메운다(bg_extract 와 같은 방식 —
        # 평균색으로 칠하면 상자가 남는다). 왼쪽이 모자라면 거울 자리에서 가져온다.
        w0 = x1 - x0
        src = a[y0:y1, x0-w0:x0] if x0 - w0 >= 0 else a[y0:y1, W-x1:W-x0][:, ::-1]
        a[y0:y1, x0:x1] = src
        print('✦ 지움 (%d,%d %dx%d) ← 옆 띠 복사' % (x0, y0, x1-x0, y1-y0))
