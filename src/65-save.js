...
function applySave(d){
  ...
  // v2.70.1: 무기 자리가 권(맨손 계열)이 되면서 옛 검·도·창·봉·철선은 EQUIP.legacyWeapon(철구)으로 옮긴다(개수 합·레벨 최대·도감 합)
  const OLDW = { sword:1, saber:1, spear:1, staff:1, fan:1 }, LW = EQUIP.legacyWeapon;
  if (d.inv || d.codex || d.itemLv || (d.equip && d.equip.weapon)){
    ...
    for (const ok in OLDW){
      if (Array.isArray(d.inv[ok])){ const t = d.inv[LW] || (d.inv[LW] = Array.from({length:NG}, () => 0)); d.inv[ok].forEach((n, i) => { t[i] = (t[i]|0) + (n|0); }); delete d.inv[ok]; }
      if (Array.isArray(d.itemLv[ok])){ const t = d.itemLv[LW] || (d.itemLv[LW] = Array.from({length:NG}, () => 0)); d.itemLv[ok].forEach((n, i) => { t[i] = Math.max(t[i]|0, n|0); }); delete d.itemLv[ok]; }
      if (d.codex[ok]){ d.codex[LW] = (d.codex[LW]|0) | (d.codex[ok]|0); delete d.codex[ok]; }
      if (d.equip.weapon && d.equip.weapon.k === ok) d.equip.weapon = Object.assign({}, d.equip.weapon, { k:LW });
    }
  }
