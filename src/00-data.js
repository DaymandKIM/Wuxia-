...
const SECT = {
  ...
  scene: {
    ...
    bgHalls: {                                   // 기와 대(4) 4채 = 사용자 sect_stage2_v1 차분(v2.92.7). 나머지 단계·산문은 그림이 오면
      yard:    { 4: { cx: 0.2386, by: 0.3467 } },
      clinic:  { 4: { cx: 0.7736, by: 0.3340 } },
      library: { 4: { cx: 0.2666, by: 0.7393 } },
      guest:   { 4: { cx: 0.7334, by: 0.7588 } },
    },
    ...
    // 전각 단계 문턱(Lv) — v2.92.8 5단계(사용자 "단계 더 뽑자"): 터 → 초가 → 목조 → 기와(소) → 기와(대). 명성 단계별 상한(5·15·30·50·80)과 맞물려
    // 명성이 한 단계 오를 때마다 전각이 한 번 자란다. 그림은 hall_<k>_<0..4>, 없는 단계는 바로 아래 있는 단계 그림으로(hallImgKey)
