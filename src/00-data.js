...
const ZONES = [
  { k:'bamboo',  n:'죽림',   ground:'#6a7a52', boss:'대나무 마왕', map:[84, 262] },
...
// [테스트 전용] 시험 패널 '보상 N일' 버튼 (v2.95.7, 사용자) — 그만큼 자리를 비운 셈 치고 정산한다(8시간 상한 없음)
// realmCap: 경지는 여기서 멈춘다 (v2.95.8, 사용자 "체력이 무한대야") — 오프라인 수련치는 '시간당 승급 수'라
// 365일이면 3285 승급이고, 주인공 체력이 1.31^3285 = Infinity 가 돼 체력바가 ∞ 로 뜬다.
// 400 이면 1.31^400 ≈ 10^47 — 말도 안 되게 세지만 숫자는 전부 유한하다.
const TESTGIVE = { days: [1, 3, 7, 15, 30, 100, 200, 365], realmCap: 400 };

// iframe(아티팩트·앱 웹뷰) 안에서 화면 맨 위에 비워 둘 높이 (v2.95.8, 사용자 "최상단 UI가 잘리고 X 버튼도 안 보인다")
// iframe 은 env(safe-area-inset-top) 을 0 으로 받는다 — 바깥이 상태바에 가려 있어도 알 길이 없다.
const SAFETOP_IFRAME = 40;
...
const EQUIP = {
  ...
  profile: {
    ...
    // 방어구 4종 (v2.94.18, 사용자 "갑옷이 제일 문제") — 옛 판은 profile 이 없어 전부 '체력 1.0 + 부가 0.4' 였고,
    // **찰갑은 부가도 체력이라 체력 1.4 한 줄**이라 넷이 사실상 같은 장비였다. 이제 넷 다 성격이 다르다(합 1.40 유지).
    robe:     { hp:0.75, regen:0.40, aspd:0.25 },   // 무복 — 가볍다. 숨 고르고 빨리 친다
    vest:     { hp:0.75, aspd:0.40, crit:0.25 },    // 피갑 — 가죽. 빠르고 급소를 노린다
    lamellar: { hp:1.15, regen:0.25 },              // 찰갑 — 무겁다. 체력 한 줄이 크다
    cloak:    { hp:0.65, spd:0.45, gold:0.30 },     // 도롱이 — 나그네. 발과 노잣돈
...
const SECT = {
  ...
  // 여정 지도 위 문파 표기 (v2.93.9 사용자 "지도에 문파도 표기") — 새 지도(zone_map 1024)에서 잰 자리, 360 기준. home = 내 문파(이름은 sectName()).
  // v2.94 본진 비무의 노드가 된다(지금은 누르면 안내 토스트만)
  hq: [                                // 내 문파(home)는 표시 안 함 — 사용자 "무명문은 표시 안 해도 될 듯"(v2.93.12). 표기 코드는 남겨 둠
    { k:'bamboo',  map:[118, 300] },
    { k:'sorim',   map:[151, 144] },
    { k:'mudang',  map:[192, 113] },
    { k:'hwasan',  map:[176, 46] },
    { k:'ami',     map:[327, 306] },
    { k:'gaebang', map:[258, 197] },
    { k:'dangmun', map:[313, 202] },
    { k:'magyo',   map:[292, 49] },
  ],
  hqR: 3.5, hqDy: 9,                   // 지도 위 문파 표식: 작은 점 반지름 · 그림 밑으로 내리는 거리 (v2.93.11 사용자 "문파가 그림을 너무 가린다" — 점+이름만, 그림 아래)
  ...
  scene: {
    ...
    // v2.92.7 사용자 그림 배치대로: 연무장 좌상·약방 우상·장경각 좌하·객당 우하, 산문은 가운데 위(돌계단 위 빈 터)
    halls: { yard:[0.224,0.294,0.224], gate:[0.502,0.335,0.220], clinic:[0.780,0.295,0.227], library:[0.278,0.724,0.220], guest:[0.728,0.723,0.222] },
    hallFit: 1.22,                               // 전각 그림 폭 ≤ 터 폭 × 이 값 (옆모습 시트 기와 165px 이 터 90px 에 서야 한다 → 배율 ≈0.67. 3/4 시트가 오면 1.0 근처로)
    // 배경 위에 얹어 받은 전각(v2.92.7, sectdiff.py 차분 추출 — review/hall_diff.json 을 그대로 옮긴다): 종류 → 단계 → {cx, by} 그림 비율.
    // 항목이 있으면 그 전각은 **배경과 같은 배율**(sectBgRect.s)로 그 자리에 그린다(옆모습 시트·hallFit 경로는 항목 없는 것만). 15개가 다 차면 옛 경로는 안 쓴다.
    // 전각 앵커(그림 비율, 가로 중앙·아랫변) — 차분(sectdiff, v2.92.7)으로 잰 값을 그 건물의 모든 단계가 같이 쓴다(v2.93 스트립도 같은 자리·같은 배율).
    // 항목이 있으면 배경 배율(sectBgRect.s)로 제자리에 그린다. 산문은 그림이 오면 가운데 위 터(0.502, ≈0.335)로
    bgHalls: {
      yard:    { cx: 0.252, by: 0.3467 },        // 차분값 0.2386 — 폭 390 폰에선 cover 크롭에 정자 왼쪽이 10px 잘려 조금 오른쪽으로(v2.93)
      clinic:  { cx: 0.7736, by: 0.3340 },
      library: { cx: 0.2666, by: 0.7393 },
      guest:   { cx: 0.7334, by: 0.7588 },
      gate:    { cx: 0.502,  by: 0.340 },        // 가운데 위 터(돌계단 위) — 차분 기준 없음, 스트립을 터 폭 1.1배(140px)로 뽑았다(v2.93.1)
    },
    hero: [0.502, 0.560],                        // 가운데 수련장, 돌길 위
    yardX: [0.31, 0.72], yardY: [0.43, 0.62],    // 제자가 거니는 띠 — 가운데 수련장 안(x 159~435 · y 406~666 의 안쪽)
    walkSpd: 0.04, discScale: 0.86,              // 제자 걸음(그림 폭 비율/s — v2.92.6 그림 기준으로)·배율(주인공보다 조금 작게)
    // 전각 단계 문턱(Lv) — 사다리 최대 5단(v2.93: 초가 → 목조 → 목조 너와 → 기와 소 → 기와 대). Lv 0 은 배경의 빈 터 그대로(이름표만, v2.92.9 "터는 빼고").
    // 전각마다 그림 수(hall_<k>_0..n-1, ASSET 로 셈)가 다르다 — 연무장 5·약방 4 — 그래서 실제 사다리는 hallLadder(k): 첫 문턱(1)은 지키고 가운데부터 뺀다
    // (n=4 → [1,15,30,50], n=3 → [1,30,50]). 명성 단계별 상한(5·15·30·50·80)과 맞물려 상한을 채울 때마다 다음 모습.
    stageLv: [1, 6, 15, 30, 50],
...
FOES.gb_elite = {
  // 개방 정예제자 — sheets/gb_elite2.png(v2.94.27 3.5등신 재작업판). **공격 두 벌**: atk 봉 찌르기(끝에 파란 기운) ·
  // atk2 회전 봉 휘두르기(파란 초승달 호). 대기 10 · 걷기 10 · 공격 각 10 · 쓰러짐 3 — 컷을 아끼지 않는다.
  // 캔버스 70×57 — 몸 26×50. DUEL.eliteFrom 단부터 섞인다.
  n:'개방 정예제자', w:70, h:57, sw:26, bh:50, school:'gaebang', elite:true,
  anim:{ idle:['idle0','idle1','idle2','idle3','idle4','idle5','idle6','idle7','idle8','idle9'],
         walk:['walk0','walk1','walk2','walk3','walk4','walk5','walk6','walk7','walk8','walk9'],
...
FOES.gb_elder = {
  // 개방 장로 — sheets/gb_elder2.png(v2.94.27 3.5등신 재작업판, 줄마다 칸 수가 다르다 8·9·10·10·10·10).
  // **공격 세 벌**: atk 지팡이 가로 휩쓸기(청록 호 10컷) · atk2 내려찍기(흰 먼지 9컷) · atk3 앞으로 찌르기(긴 청록 줄기 7컷).
  // 캔버스 90×62 — 몸 36×58. 보스 틀(range 는 BOSS.range).
  n:'개방 장로', w:90, h:62, sw:36, bh:58, school:'gaebang',
  anim:{ idle:['idle0','idle1','idle2','idle3','idle4','idle5','idle6','idle7'],
         walk:['walk0','walk1','walk2','walk3','walk4','walk5','walk6','walk7'],
         atk:['atk0','atk1','atk2','atk3','atk4','atk5','atk6','atk7','atk8','atk9'],
...
FOES.hs_elder = { n:'화산 장로',     w:72, h:71, sw:38, bh:59, school:'hwasan',
  anim:{ idle:['idle0','idle1','idle2','idle3','idle4','idle5','idle6','idle7','idle8','idle9','idle10','idle11'],
         walk:['walk0','walk1','walk2','walk3','walk4','walk5','walk6','walk7','walk8','walk9'],
         atk:['atk0','atk1','atk2','atk3','atk4','atk5','atk6','atk7','atk8','atk9'],
         atk2:['atk2_0','atk2_1','atk2_2','atk2_3','atk2_4','atk2_5','atk2_6','atk2_7','atk2_8','atk2_9'],
...
FOES.mg_disc = { n:'마교 수습제자', w:56, h:50, sw:22, bh:49, school:'magyo',
...
// 마교 정예제자 — sheets/mg_elite2.png(v2.94.28 3.5등신 재작업판). **공격 두 벌**: atk 머리 위 내려찍기(진홍 초승달) ·
// atk2 백핸드 후리기(진홍 호). 대기 10 · 걷기 10 · 공격 9씩 · 쓰러짐 5. 캔버스 64×58 — 몸 26×51(대기 폭 34 는 칼을 가로로 든 것).
FOES.mg_elite = { n:'마교 정예제자', w:64, h:58, sw:26, bh:51, school:'magyo', elite:true,
  anim:{ idle:['idle0','idle1','idle2','idle3','idle4','idle5','idle6','idle7','idle8','idle9'], walk:['walk0','walk1','walk2','walk3','walk4','walk5','walk6','walk7','walk8','walk9'],
         atk:['atk0','atk1','atk2','atk3','atk4','atk5','atk6','atk7','atk8'],
         atk2:['atk2_0','atk2_1','atk2_2','atk2_3','atk2_4','atk2_5','atk2_6','atk2_7','atk2_8'],
         hit:['hit'], death:['hit','death0','death1','death2','death3','death4'] },
...
// 캔버스 54×53 — 몸 26×50.
FOES.am_disc = { n:'아미 수습제자', w:54, h:53, sw:22, bh:49, school:'ami',
  anim:{ idle:['idle0','idle1','idle2','idle3','idle4','idle5','idle6','idle7','idle8','idle9'],
         walk:['walk0','walk1','walk2','walk3','walk4','walk5','walk6','walk7','walk8','walk9'],
         atk:['atk0','atk1','atk2','atk3','atk4','atk5','atk6','atk7','atk8','atk9'],
         hit:['hit'], death:['hit','death0','death1','death2','death3','death4'] },
...
DUEL.hqDesc = {
  ...
  mudang:  '도가의 산문 — 부드러움이 굳셈을 이긴다',
  hwasan:  '매화검의 산 — 검이 꽃처럼 피어난다',
  ami:     '아미산의 비구니 — 자비와 검이 함께 간다',
  dangmun: '사천 당가 — 암기와 독의 주인',
  magyo:   '정파가 마(魔)라 부르는 이들',
  bamboo:  '대숲의 작은 문파 — 네가 자란 곳',
};
