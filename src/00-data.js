...
const SECT = {
  ...
  scene: {
    ...
    bgHalls: {                                   // 기와 대(3) 4채 = 사용자 sect_stage2_v1 차분(v2.92.7). 나머지 단계·산문은 그림이 오면
      yard:    { 3: { cx: 0.2386, by: 0.3467 } },
      clinic:  { 3: { cx: 0.7736, by: 0.3340 } },
      library: { 3: { cx: 0.2666, by: 0.7393 } },
      guest:   { 3: { cx: 0.7334, by: 0.7588 } },
    },
    ...
    // 전각 단계 문턱(Lv) — v2.92.9 4단계(사용자 "터는 빼고 바로 짓자"): 초가 → 목조 → 기와(소) → 기와(대). Lv 0 은 배경의 빈 터 그대로(이름표만).
    // 명성 단계별 상한(무명 5·향리 15·일방 30·명문 50)에 맞물려 상한을 채울 때마다 다음 모습으로. 그림은 hall_<k>_<0..3>, 없는 단계는 바로 아래 단계 그림으로(hallImgStage)
    stageLv: [1, 15, 30, 50],
